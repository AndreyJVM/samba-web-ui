...
class SambaUserServiceTest {
    ...
    void getAllUsers_ShouldParsePdbeditOutputCorrectly() throws Exception {
        ...
        when(commandExecutor.execute(SESSION_ID, LinuxCommands.listSambaUsers())).thenReturn(mockOutput);
    ...
    void createUser_ShouldExecuteCorrectSequenceOfCommands() throws Exception {
        ...
        verify(commandExecutor).execute(SESSION_ID, LinuxCommands.addSystemUserWithHome(username, fullName));
        verify(commandExecutor).execute(eq(SESSION_ID), eq(LinuxCommands.chpasswd()), eq(username + ":" + password + "\n"));
        verify(commandExecutor).execute(eq(SESSION_ID), eq(LinuxCommands.addSambaUser(username)), eq(password + "\n" + password + "\n"));
    ...
    void deleteUser_ShouldHandleExceptionsGracefullyForSambaAndKillSystemUser() throws Exception {
        ...
        // Метод не должен выбросить исключение наружу, он должен проигнорировать первичное и удалить системного юзера
    ...
    void changePassword_ShouldUpdateBothSystemAndSamba() throws Exception {
        ...
        verify(commandExecutor).execute(eq(SESSION_ID), eq(LinuxCommands.chpasswd()), eq("bob:newpassword!\n"));
        verify(commandExecutor).execute(eq(SESSION_ID), eq(LinuxCommands.changeSambaPassword(username)), eq("newpassword!\nnewpassword!\n"));
    }
}
