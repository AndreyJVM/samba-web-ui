...
@AutoConfigureMockMvc(
    addFilters = false) // Disable security constraints for AuthController explicitly to simulate
// unauthenticated access tests easily
class AuthControllerTest {
  ...
  void connect_WithInvalidCredentials_ShouldReturnIndexWithError() throws Exception {
    doThrow(new RuntimeException("Auth failed"))
        .when(sessionManager)
        .createSession(anyString(), any(ConnectionRequestDto.class));
  ...
  void disconnect_ShouldInvalidateSessionAndRedirect() throws Exception {
    ...
    mockMvc
        .perform(get("/disconnect").session(session))
        .andExpect(status().is3xxRedirection())
