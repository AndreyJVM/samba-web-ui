...
class MonitoringControllerTest {
  ...
  void dashboard_ShouldReturnStatusTemplate() throws Exception {
    ...
    // The dashboard template expects diskUsage mapping: diskUsage.free which crashes because we
    // mock it to return null/empty. Let's fix that.
  ...
  void killSession_ShouldRedirectToStatus() throws Exception {
    ...
    mockMvc
        .perform(post("/status/kill").with(csrf()).session(session).param("pid", "12345"))
