...
public final class LinuxCommands {
    ...
    public static String findDirectories(String path) {
        return String.format("sudo find %s -mindepth 1 -maxdepth 1 -type d ! -name '.*' 2>/dev/null | sort", escape(path));
    }
    ...
    public static String listBackupsDetailed(String backupDir) {
        return String.format("ls -lh --time-style=\"+%%Y-%%m-%%d %%H:%%M:%%S\" %s/smb.conf.backup_* 2>/dev/null", backupDir);
    }

    public static String cleanupOldBackups(String backupDir, int keepCount) {
        return String.format("ls -t %s/smb.conf.backup_* 2>/dev/null | tail -n +%d | xargs -r sudo rm --", backupDir, keepCount + 1);
    }
    ...
    public static String addSystemUserWithHome(String username, String comment) {
        return String.format("sudo useradd -m -s /bin/bash -c %s %s", escape(comment), escape(username));
    }
