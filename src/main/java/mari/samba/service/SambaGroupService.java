 ...
 *
 * <p>Все группы, создаваемые через приложение, получают префикс {@code smb_}. Это позволяет
 * безопасно изолировать их от системных сервисных групп ОС (таких как root, daemon, sys) и избежать
 * случайных поломок системы. При выводе в пользовательский интерфейс префикс скрывается.
 */
@Service
