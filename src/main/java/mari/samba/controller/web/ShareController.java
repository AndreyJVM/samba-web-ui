...
public class ShareController {
    ...
    public String createShare(HttpSession session, @ModelAttribute SambaShareCreateDto dto, RedirectAttributes redirectAttributes) throws Exception {
        String sessionId = session.getId();
        shareService.createShare(sessionId, dto);
        redirectAttributes.addFlashAttribute("successMessage", "Общая папка '" + dto.getName() + "' создана!");
        return "redirect:" + WebRoutes.SHARES;
    }

    @GetMapping(WebRoutes.SHARES_EDIT_MAPPING)
    public String showEditForm(HttpSession session, @PathVariable String sharename, Model model) throws Exception {
        String sessionId = session.getId();
    ...
    public String updateShare(HttpSession session, @PathVariable String sharename, @ModelAttribute SambaShareCreateDto dto, RedirectAttributes redirectAttributes) throws Exception {
        String sessionId = session.getId();
        dto.setName(sharename); // Принудительно сохраняем имя, так как оно в пути
        shareService.updateShare(sessionId, sharename, dto);
        redirectAttributes.addFlashAttribute("successMessage", "Настройки папки '" + sharename + "' обновлены!");
        return "redirect:" + WebRoutes.SHARES;
    }

    @PostMapping(WebRoutes.SHARES_DELETE_MAPPING)
    public String deleteShare(HttpSession session, @RequestParam String sharename, RedirectAttributes redirectAttributes) throws Exception {
        String sessionId = session.getId();
