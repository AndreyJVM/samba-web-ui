...
export default function ConfigPage() {
  ...
  const fetchConfig = async () => {
    try {
      setLoading(true);
      const res = await api.get<SmbGlobalConfig>("/api/config");
      setConfig(res);
    } catch (err: any) {
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4">
        ...
        <div className="flex gap-2">
          <button onClick={handleReset} type="button" className="text-[13px] font-medium text-foreground bg-surface border border-border px-4 py-2.5 rounded-md hover:bg-surface-hover shadow-sm-subtle transition-colors flex items-center gap-2">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="bg-surface rounded-lg shadow-sm-subtle border border-border overflow-hidden">
           ...
           <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.workgroup")}</label>
                <input 
                  value={config.workgroup} onChange={(e) => setConfig({...config, workgroup: e.target.value})} 
                  className="w-full bg-surface border border-border focus:border-border-strong focus:outline-none focus:ring-4 focus:ring-ring text-foreground transition-all py-2.5 px-3.5 text-[13px] font-medium rounded-lg shadow-sm-subtle uppercase"
                />
              </div>
              <div className="space-y-2">
                <label className="flex flex-col gap-0.5">
                  <span className="text-[12px] font-medium text-foreground">{t("config.netbiosName")}</span>
                </label>
                <input 
                  value={config.netbiosName} onChange={(e) => setConfig({...config, netbiosName: e.target.value})} 
                  className="w-full bg-surface border border-border focus:border-border-strong focus:outline-none focus:ring-4 focus:ring-ring text-foreground transition-all py-2.5 px-3.5 text-[13px] font-medium rounded-lg shadow-sm-subtle"
                  placeholder={t("config.autoGenerated")}
                />
              </div>
              <div className="space-y-2 md:col-span-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.serverString")}</label>
                <input 
        ...
        <div className="bg-surface rounded-lg shadow-sm-subtle border border-border overflow-hidden">
           ...
           <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.interfaces")}</label>
                <input 
        ...
        <div className="bg-surface rounded-lg shadow-sm-subtle border border-border overflow-hidden">
           ...
           <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.securityModel")}</label>
                <select 
                  value={config.security} onChange={(e) => setConfig({...config, security: e.target.value})}
                  className="w-full bg-surface border border-border focus:border-border-strong focus:outline-none focus:ring-4 focus:ring-ring text-foreground transition-all py-2.5 px-3.5 text-[13px] rounded-lg shadow-sm-subtle appearance-none cursor-pointer"
                >
              ...
              <div className="space-y-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.mapToGuest")}</label>
                <select 
              ...
              <div className="space-y-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.minProtocol")}</label>
                <select 
              ...
              <div className="space-y-2">
                <label className="text-[12px] font-medium text-foreground">{t("config.maxProtocol")}</label>
                <select 
        ...
        <div className="flex justify-end pt-4 border-t border-border">
          <button 
            type="submit" 
            disabled={saving}
            className="text-[13px] font-medium text-brand-text bg-brand px-6 py-2.5 rounded-md hover:bg-brand-hover shadow-sm transition-colors flex items-center gap-2 disabled:opacity-50"
          >
